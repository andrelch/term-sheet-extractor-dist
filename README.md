...
- PostgreSQL 18 (x64)
- Node.js 22 LTS (x64) — needed to verify and install the first release; installed releases use
  their own bundled, tested `runtime\node.exe`
- [NSSM](https://nssm.cc/) — runs the services
- [Caddy](https://caddyserver.com/) — terminates HTTPS

You'll also need an elevated PowerShell session and a DNS hostname pointed at this machine. Normally,
two people should be available to hold escrow certificates. If the organization has not appointed
them yet, Step 4 provides an explicit temporary deferred-escrow procedure.
...
copy and can continue to Step 1. Otherwise, open PowerShell in the directory where you want to keep
...
Telegram and browser notification credentials follow the same ownership choice. In encrypted mode,
a `data-admin` configures them under **Settings -> Notifications** and the server generates the
webhook/VAPID secrets. In environment mode, IT supplies the documented `TELEGRAM_*`, `APP_BASE_URL`,
and `VAPID_*` values and restarts web and worker services.
